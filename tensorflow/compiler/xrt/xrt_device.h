 ...
 public:
  ...
  class ScopedRef {
   ...
   private:
    ...
    void Acquire(xla::LocalClient* client, int ordinal, const std::string& platform_name) {
      client_ = client;
      ordinal_ = ordinal;
    ...
    static mutex mutex_;
    static std::map<int64, std::unique_ptr<se::TfAllocatorAdapter>> compile_cuda_allocators_;
    static std::map<std::string, std::unique_ptr<se::TfAllocatorAdapter>> other_cuda_allocators_;
  };
