 ...
 public:
  ...
  class WorkingSet {
   public:
    ...
    Status LookupAndPin(xla::Backend* backend, int64 handle, se::DeviceMemoryAllocator* allocator);
...
xla::StatusOr<T> XRTMemoryManager::Run(
    ...
    int device_ordinal, size_t requested_free_size, se::DeviceMemoryAllocator* allocator) {
  ...
  while (true) {
    ...
    TF_RETURN_IF_ERROR(TryFreeMemoryStep(&mrctx, result_or.status(), allocator));
  }
}
